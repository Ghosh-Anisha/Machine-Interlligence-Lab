...
class AdaBoost:
    ...
    def stump_error(self, y, y_pred, sample_weights):
        ...
        for i in range(0,len(y)):
            if(y[i]!=y_pred[i]):
                st_error+=1*sample_weights[i]
        st_error=st_error/np.sum(sample_weights)
        return st_error
    ...
    def compute_alpha(self, error):
        ...
        """
        eps = 1e-9
        alpha=np.log((1-error+eps)/(error+eps))
        alpha =alpha/2
        return alpha
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        ns_067=4*(1-error)*(error)
        if(ns_067==0):
            ns_067=1
        ns_067=ns_067**0.5
        new_sample_weight_067=[]
        if(error==0):
            return sample_weights
        else:
            for i in range(len(y)):
                if(y[i]==y_pred[i]):
                    new_sample_weight_067.append(sample_weights[i]/(2*(1-error)))
                else:
                    new_sample_weight_067.append(sample_weights[i]/(2*error))
            
        return new_sample_weight_067
    ...
    def predict(self, X):
        ...
        for i in range(self.n_stumps):
            prob.append(self.stumps[i].predict(X))
        np.array(prob)
        #print(prob)
        return np.sign(prob[0])
